...
void OutputFTLEtoFile(int ss) {
  ...
  double *FTLEarray = new (nothrow) double[(ND+1)*FTLE_BlockSize];
  if(FTLEarray == NULL) {
  ...
  fin.read((char *)&FTLEarray[0],(ND+1)*FTLE_BlockSize*sizeof(double));
  fin.close();
  ...
  fout.write((char *)FTLEarray,(ND+1)*FTLE_BlockSize * sizeof(double));
...
void ComputeFTLE(int ss, long int j1, long int packetID) {
  ...
  double *pt_FTLE;
  double *pt_CGeigenvector;
  pt_FTLE = new double;  
  pt_CGeigenvector = new double[ND];
  for(int f=0 ; f < FTLE[ss].blocksize; f++) {
    
    if(!(FTLE_Pts[f].HaveFTLE))
	  GetFTLEForPoint(j1+f, FTLE_IntTLength, pt_FTLE, pt_CGeigenvector);
      FTLE_Pts[f].FTLEValue = *pt_FTLE;
	  //cout << FTLE_Pts[f].FTLEValue << endl;
	  for(int i=0; i<ND; i++){
		  FTLE_Pts[f].CGeigenvector[i] = pt_CGeigenvector[i];
		  //cout << FTLE_Pts[f].CGeigenvector[i] << endl;
	  }
  }
  delete pt_FTLE;
  delete[] pt_CGeigenvector;
  ...
  */
  double *tmpd = new double[(ND+1)*FTLE[ss].blocksize];
  int index = 0;
  for(int ii=0;ii<FTLE[ss].blocksize;++ii){
		tmpd[ND*ii+index]=FTLE_Pts[ii].FTLEValue;
		//cout << ND*ii+index << endl;
		for(int jj=0;jj<ND;jj++){
			tmpd[ND*ii+jj+1+index] = FTLE_Pts[ii].CGeigenvector[jj];
			//cout << ND*ii+jj+1+index << endl;
		}
		index++;
  }
 
  fout.write((char *)&tmpd[0],(ND+1)*FTLE[ss].blocksize*sizeof(double));
...
void GetFTLEForPoint(int f0, double FTLEtime, double *pt_FTLEvalue, double *pt_CGeigenvector) {
  
  if(FTLEtime < 0.001 * FTLE_IntTLength) {
	cout << "FTLE Time is too low, FTLE.CPP line 909" << endl;
    *pt_FTLEvalue=0.0;
	return;
  }
  else {
    ...
    if(FTLE_Type == 0) {
	  ...
	  */
	  // Get eigenvalues of Delta
	  gsl_eigen_symmv_workspace *eigenworkspace = gsl_eigen_symmv_alloc(ND);
	  gsl_vector *eigenvals = gsl_vector_alloc(ND);
	  gsl_matrix *eigenvecs = gsl_matrix_alloc(ND,ND);
	  int status = gsl_eigen_symmv(Delta_gsl,eigenvals,eigenvecs,eigenworkspace);
	  
      if( status != GSL_SUCCESS ) gsl_strerror(status);
      
      lambda =  gsl_vector_max(eigenvals);
	  //cout << lambda <<endl;
      
	  status = gsl_eigen_symmv_sort (eigenvals, eigenvecs, GSL_EIGEN_SORT_VAL_DESC);
	  
	  if( status != GSL_SUCCESS ) gsl_strerror(status);
	  
	  lambda = gsl_vector_get(eigenvals,0);	
	  //cout << lambda << endl;
	  //gsl_vector_view eigenvec_1 = gsl_matrix_column(eigenvecs,0);
	  for(int i=0; i<ND; i++){
		pt_CGeigenvector[i] = gsl_matrix_get(eigenvecs,i,0);
	  }
	  
	  
	  
	  //gsl_vector_fprintf(stdout, &eigenvec_1.vector, "%g");
      ...
      */
	  ...
	  //*pt_FTLEvalue = lambda;
	  *pt_FTLEvalue = 0.5*log(lambda)/FTLEtime;
	  return;
	  
	  /*
      if(lambda>1.0) {  // lambda > 0.0   ?????????
	    *pt_FTLEvalue = 0.5*log(lambda)/FTLEtime;
        return ;   // Option to divide by FTLE_IntTLength or FTLEtime here
        //return 0.5*log(lambda);
      }
      else {
		*pt_FTLEvalue = 0.0;
