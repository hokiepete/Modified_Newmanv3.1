...
struct _FieldData_a
  {
     ...
     *   The following two members may NEVER move as VariableClientData is
     *   accessed by add-ons performing custom load-on-demand actions. Add-ons
     *   access the VariableClientData member via a macro that always expects
     *   it to be at the second pointer position of the field data pointer. The
     *   first pointer position is reserverd for Tecplot's own Data pointer for
     *   fastest access.  See TecUtilDataValueCustomLOD() for details.
     */
    void       *Data;               /* !!! NEVER MOVE THIS !!! */
    ArbParam_t  VariableClientData; /* !!! NEVER MOVE THIS !!! */

    /* PRIVATE */
    FieldValueGetFunction_pf   GetFunction;
    FieldValueSetFunction_pf   SetFunction;
    FieldDataType_e            Type;
    Boolean_t                  IsNativeOrdering; /* mem-mapped may not be in native order */
    Boolean_t                  IsWordAligned;    /* mem-mapped may not be byte aligned */
 ...
 */
  ...
  #define FieldDataHasContiguousAddressSpace FieldDataHasContiguousAddressSpace_MACRO
#else
  ...
  #define FieldDataHasContiguousAddressSpace FieldDataHasContiguousAddressSpace_FUNC
#endif

#define GetFieldDataType_MACRO(FieldData)          ((FieldData)->Type)
#define GetFieldDataGetFunction_MACRO(FieldData)   ((FieldData)->GetFunction)
#define GetFieldDataSetFunction_MACRO(FieldData)   ((FieldData)->SetFunction)
...
/* CORE SOURCE CODE REMOVED */
#else
  #define FieldDataHasContiguousAddressSpace_MACRO(FieldData) ((FieldData)->Data != NULL)
#endif
  ...
  EXTERN Boolean_t FieldDataHasContiguousAddressSpace_FUNC(FieldData_pa FieldData);
#endif
 ...
 */
  ...
  EXTERN double STDCALL GetFieldValueForFloat(const FieldData_pa fd,
                                              LgIndex_t          pt);
  ...
  EXTERN double STDCALL GetFieldValueForDouble(const FieldData_pa fd,
                                               LgIndex_t          pt);
