  ...
  if (UseActiveAllocLog)
    {
      size_t ii;
      size_t startsize;
      for (ii = 0; ii < NumActiveAllocs; ii++)
        {
          void *adjptr;
          char *endptr;
          adjptr = STARTSIZEADDR(ActiveAllocs[ii]);
          startsize = *(size_t *)adjptr;
          endptr = ENDADDR(ActiveAllocs[ii],ActiveAllocSizes[ii]);
...
{
  char *DebugAllocLogFileName;

  REQUIRE(!DebugAllocInitialized && DebugAllocLogFile == NULL);

  DebugAllocLogFileName = GETENV("DEBUGALLOCLOGFILE");
                ...
                const char  *desc)
{
  ...
  DAmount = ALLOCAMOUNT((double)size);
  if (DAmount > MAXINDEX)
    ptr = NULL;
  else
                                ...
                                size_t NumBytes)
{
  size_t Word;
  size_t Byte;
  size_t NumBytesPer32BitWord;
  size_t Num32BitWords;
  size_t NumBytesRemaining;

  REQUIRE(VALID_REF(Address));
  REQUIRE(NumBytes >= 1);

  NumBytesPer32BitWord = 4;
  Num32BitWords = NumBytes / NumBytesPer32BitWord;
  NumBytesRemaining = NumBytes % NumBytesPer32BitWord;

  for (Word = 0; Word < Num32BitWords; Word++)
    {
  ...
  for (Byte = 0; Byte < NumBytesRemaining; Byte++)
    {
...
#if defined DEBUG_X_ALLOCATIONS
/**
 */
void *Xmalloc(size_t size)
{
  ...
  if (size == 0)
    ...
    result = newmalloc(size, "file", 1, "desc");
  ...
  if (ptr != NULL)
    {
      if (size == 0)
        ...
        {
          size_t b;
          size_t old_size = *((size_t *)(STARTSIZEADDR(ptr)));
          result = Xmalloc(size);
          if (result != NULL)
            for (b = 0; b < size && b < old_size; b++)
              ((char *)result)[b] = ((char *)ptr)[b];
...
void MSWinFree(void *pMem)
{
...
#if defined MSWIN && defined MSWIN_DEBUG_ALLOC
  unsigned char *mswin_debug_alloc(int N,
                                   const char *Type,
                                   const char *file,
                                   int Line,
                                   const char *desc)
  ...
  void mswin_debug_free(unsigned char *X,
                        const char *file,
                        int Line,
                        const char *desc)
  {
