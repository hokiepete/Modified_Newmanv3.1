      ...
      if (IsOk)
        {
          char *S = NULL;
          int  INumVars;
          IsOk = ReadInDataFileTitleAndVarNames(FileStream,
                                                IVersion,
                                                ((Pass == 2) ? &S : (char **)NULL),
                                                &INumVars,
                      ...
                      if (IsOk)
                      ...
                      else
                        {
                          if (ZoneSpec->ParentZone == *NumZones)
                            ErrMsg(_ts("Parent zone assignment for zone %d "
...
/* CORE SOURCE CODE REMOVED */
              ...
              else if (X1 == DataSetAuxMarker)
                {
                  ...
                  if (!IsOk)
                    {
                      ErrMsg(_ts("Invalid DATASETAUXDATA record in binary datafile"));
                      SentError = TRUE;
                    }
                }
                  ...
                  if (IsOk)
                    {
                      ...
                      if (!IsOk)
                        {
                          ErrMsg(_ts("Invalid VARAUXDATA record in binary datafile"));
                          SentError = TRUE;
                        }
                    }
                  else
                    {
                      ErrMsg(_ts("Invalid VARAUXDATA variable number association"));
                      SentError = TRUE;
                    }
                }
              else
...
/* CORE SOURCE CODE REMOVED */
  ...
  if (!IsOk && !SentError)
    ErrMsg(_ts("Invalid header in binary datafile"));
  ...
  if (Result)
    {
      *IVersion = GetInputVersion(*FileStream);
       ...
       */
      CHECK(TecplotBinaryFileVersion == 108);
    }
