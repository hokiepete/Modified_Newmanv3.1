 ...
 *****************************************************************
 *******                                                  ********
 ****** (C) Copyright 1989-2002  by AMTEC ENGINEERING INC.********
 *******       All Rights Reserved.                       ********
 *******                                                  ********
 *****************************************************************
 *****************************************************************
  ...
  if (Result != NULL)
    {
      ...
      Result->Name   = DupString(_nts(Name));
      IsOk = (Result->Name != NULL);
      Result->Value = 0; /* to satisfy some compilers' uninitialized warning */
      if (IsOk && Type == AuxDataType_String)
        {
          if ((char *)Value != NULL)
            {
              Result->Value = (ArbParam_t)DupString(_nts((const char*)Value));
              IsOk = ((char *)Result->Value != NULL);
            }
          else
