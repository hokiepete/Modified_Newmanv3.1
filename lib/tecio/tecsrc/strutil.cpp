...
#         if defined MSWIN
          // Windows sprintf with utf8 sometimes doesn't add 
          // the trailing \0, so be sure that the buffer is
          // zero'ed out
...
#         endif
      ...
      if (FormatStringBuffer != NULL)
        Result = DupString(_nts(FormatStringBuffer));
    }
...
/**
 */
char *FormatString(TranslatedString_pa Format,
                   ...) /* 0 or more variable arguments */
{
  ...
  Result = vFormatString(_ts2char(Format), Arguments);
  va_end(Arguments);
 ...
 */
char *DupString(TranslatedString_pa String)
{
  char *Result;

  REQUIRE(VALID_REF(String));

  Result = ALLOC_ARRAY(strlen(_ts2char(String)) + 1, char, "duplicate string");
  if (Result != NULL)
    strcpy(Result, _ts2char(String));

  ENSURE(Result == NULL || (VALID_REF(Result) && strcmp(Result, _ts2char(String)) == 0));
  return Result;
}
  ...
  if (Result != NULL)
    {
      ...
      if (strlen(String) > WrapMargin)
        {
          char *StringCopy = DupString(_nts(String));
...
#endif
{
  ...
  if ( NewString == NULL )
    ...
    {
#if defined UNIXX && !defined NO_ASSERTS && defined DEBUG_ALLOC
      char S[80+1];
      MakeDebugRecord(FileName, LineNumber, "duplicating", NewString, S, 80);
      *SBase = ALLOC_ARRAY(strlen(NewString)+1,char,S);
#elif defined MSWIN && defined _DEBUG
      string strDebug = string("duplicating:") + NewString;
      *SBase = ALLOC_ARRAY(strlen(NewString)+1,char,strDebug.c_str());
#else
...
{
  REQUIRE(VALID_REF(SBase));
  REQUIRE(VALID_UTF8_SBCHAR(CharToAdd));
