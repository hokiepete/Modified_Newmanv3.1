          ...
          for (Z = 0; Z < *NumZones && InputIsOk; Z++)
            {
              ...
              if (!InputIsOk)
                ErrMsg(_ts("Cell centered data not supported by ReadTec."));
            }
        }
    }
      ...
      if ((*NumZones > 0) && !RawDataSpaceAllocated)
        {
          *VDataBase   = ALLOC_ARRAY(*NumZones*(*NumVars),double *,"vdatabase array");
          if (*VDataBase == NULL) 
            {
              ErrMsg(_ts("Cannot allocate space for field data"));
              InputIsOk = FALSE;
            }
          else
            {
          ...
          if (InputIsOk)
            {
              *NodeMap = ALLOC_ARRAY(*NumZones,NodeMap_t *,"nodemap array");
              if (*NodeMap == NULL) 
                {
                  ErrMsg(_ts("Cannot allocate space for nodemap"));
                  InputIsOk = FALSE;
                }
              else
                {
              ...
              if (*IVersion > 45 && *IVersion < 101 && InputIsOk)
                {
                  ...
                  if ((NumDupVars > 0) && (CurZone == 0))
                    {
                      ErrMsg(_ts("Cannot duplicate variables from the first zone since there are "
                  ...
                  for (J = 0; J < *NumVars; J++)
                    {
                      ...
                      if (!InputIsOk)
                        {
                          ErrMsg(_ts("Invalid data type - binary input file corrupted"));
                          InputIsOk = FALSE;
                        }
                    }
                }
              ...
              if (InBlockFormat)
                {
                  ...
                  if (!InputIsOk)
                    ErrMsg(_ts("Invalid raw data section of binary file"));
                }
                      ...
                      while (InputIsOk && (N < TotalNumPts))
                        {
                          ...
                          if (!InputIsOk)
                            ErrMsg(_ts("Binary datafile corrupted!"));
                          N++;
                        }
                    }
                }
                  ...
                  if (!RawDataSpaceAllocated && NumKPts*NumJPts >= 1)
                    {
                      ...
                      if ((*NodeMap)[CurZone] == NULL)
                        ErrMsg(_ts("Cannot allocate space for connectivity list",
                  ...
                  if (InputIsOk && ConnectivitySharesFromZone[CurZone] != -1)
                    {
                      ...
                      if (SourceZone >= CurZone)
                        {
                          ErrMsg(_ts("Zone %d is attempting to share connectivity "
                      ...
                      while (Connection < CurZoneSpec->UserDefFNConnections && InputIsOk)
                        {
                          ...
                          if (!InputIsOk)
                            ErrMsg(_ts("Unexpected end-of-file while reading face neighbor data."));
                          ...
                          if (InputIsOk)
                            {
                              ...
                              if (!InputIsOk)
                                ErrMsg(_ts("Corrupt input file: invalid face neighbors."));
                            }
                        }
                    }
                }
